...
function sanitizePersistedState(state: unknown): Partial<AppStoreState> {
  ...
  return {
    ...
    useRagForConversation: typeof candidate.useRagForConversation === 'boolean' ? candidate.useRagForConversation : false,
...
function getNextFontSizeMode(current: FontSizeMode, direction: 'increase' | 'decrease'): FontSizeMode {
  const modes: FontSizeMode[] = ['compact', 'normal', 'large'];
  const index = modes.indexOf(current);
  if (direction === 'increase') {
    return modes[Math.min(index + 1, modes.length - 1)] ?? 'large';
  }
  return modes[Math.max(index - 1, 0)] ?? 'compact';
}
...
export const useAppStore = create<AppStoreState>()(
  persist(
    (set) => ({
      ...
      increaseFontSize: () => set((state) => ({ fontSizeMode: getNextFontSizeMode(state.fontSizeMode, 'increase') })),
      decreaseFontSize: () => set((state) => ({ fontSizeMode: getNextFontSizeMode(state.fontSizeMode, 'decrease') })),
