...
Matériel de test : **Apple M5 16GB** — modèle LM Studio : `google/gemma-4-e4b` (Q4)
...
Limite : LM Studio est sollicité en continu — le Mac reste moins disponible pour d'autres usages.
...
### Modèles LM Studio recommandés par configuration

| RAM | Modèle recommandé | Qualité | Tok/s attendus |
| --- | --- | --- | --- |
| 16GB | `google/gemma-4-e4b` (Q4) | Bonne | 40–60 |
| 16GB | `qwen2.5-7b-instruct` (Q4_K_M) | Bonne | 50–70 |
| 24GB | `qwen2.5-14b-instruct` (Q4_K_M) | Très bonne | 40–60 |
| 48GB | `qwen2.5-14b-instruct` (Q6_K) | Excellente | 60–80 |
| 48GB | `mistral-nemo-12b` (Q8) | Excellente | 70–90 |
| 128GB | `qwen2.5-72b-instruct` (Q4_K_M) | Maximale | 30–50 |
...
- Les durées ObsiRAG incluent les pauses configurées entre les appels LLM (`_SLEEP_BETWEEN_NOTES`, `_SLEEP_BETWEEN_QUESTIONS`) pour ne pas saturer LM Studio.
- La recherche web (DDG + fetch URLs) prend ~30–45s par question, indépendamment du matériel.
- Les embeddings (`sentence-transformers`) tournent sur CPU/ANE et ne dépendent pas de LM Studio — l'indexation initiale d'un coffre de 100 notes prend ~2–5 min quel que soit le matériel.
